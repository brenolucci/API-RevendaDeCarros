...
class Modelos
{
    ...
    public function buscarPorMarca($marca_id): array
    {
        $sql = 'SELECT nome FROM modelos WHERE marca_id = ' . $marca_id . ' LIMIT 200';
        $result = $this->conn->query($sql);
        while ($row = $result->fetch_assoc()) {
            $modelos[] = ($asArray = true) ? $this->toArray($this->montaModelos($row)) : $this->montaModelos($row);
        }
        return $modelos;
    }
